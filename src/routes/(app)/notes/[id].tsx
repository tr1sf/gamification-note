              ...
              <Switch>
                <Match when={!isEditing()}>
                  <article>
                    ...
                    {/* Quality Score — shown at top for immediate feedback */}
                    <Show when={n().isOwner}>
                    ...
                    <Show when={n().aiSummary}>
                      <div class="mt-8 p-4 bg-surface-elevated rounded-lg border border-surface-border">
                        ...
                        <ul class="text-sm text-ink-primary space-y-1.5">
                          {n().aiSummary!.split("\n").filter((line: string) => line.trim()).map((line: string) => {
                            const text = line.replace(/^[-*•]\s*/, "").trim();
                            return <li class="flex gap-2"><span class="text-accent shrink-0">•</span><span>{text}</span></li>;
                          })}
                        </ul>
                      </div>
                    </Show>
      ...
      {(q) => (
        <div class="mt-8 p-4 bg-surface-elevated rounded-lg border border-surface-border">
          ...
          <div class="flex flex-wrap gap-1.5 mb-3">
            <span class={`text-xs px-2 py-0.5 rounded ${q().breakdown.heading ? "bg-success-bg text-success" : "bg-surface-border text-ink-tertiary"}`}>
              {q().breakdown.heading ? "✓" : "✗"} Heading
            </span>
            <span class={`text-xs px-2 py-0.5 rounded ${q().breakdown.subheading ? "bg-success-bg text-success" : "bg-surface-border text-ink-tertiary"}`}>
              {q().breakdown.subheading ? "✓" : "✗"} Subheading
            </span>
            <span class={`text-xs px-2 py-0.5 rounded ${q().breakdown.list ? "bg-success-bg text-success" : "bg-surface-border text-ink-tertiary"}`}>
              {q().breakdown.list ? "✓" : "✗"} List
            </span>
            <span class={`text-xs px-2 py-0.5 rounded ${q().breakdown.code ? "bg-success-bg text-success" : "bg-surface-border text-ink-tertiary"}`}>
              {q().breakdown.code ? "✓" : "✗"} Code
            </span>
            <span class={`text-xs px-2 py-0.5 rounded ${q().breakdown.links ? "bg-success-bg text-success" : "bg-surface-border text-ink-tertiary"}`}>
              {q().breakdown.links ? "✓" : "✗"} Links
            </span>
            <span class={`text-xs px-2 py-0.5 rounded ${q().breakdown.tagsCategory ? "bg-success-bg text-success" : "bg-surface-border text-ink-tertiary"}`}>
              {q().breakdown.tagsCategory ? "✓" : "✗"} Tags/Category
            </span>
            <span class={`text-xs px-2 py-0.5 rounded ${q().breakdown.words50 ? "bg-success-bg text-success" : "bg-surface-border text-ink-tertiary"}`}>
              {q().breakdown.words50 ? "✓" : "✗"} 50+ words
            </span>
            <span class={`text-xs px-2 py-0.5 rounded ${q().breakdown.words200 ? "bg-success-bg text-success" : "bg-surface-border text-ink-tertiary"}`}>
              {q().breakdown.words200 ? "✓" : "✗"} 200+ words
            </span>
          </div>

          {/* Improvement tips */}
          <Show when={q().improvements?.length > 0}>
            <div class="border-t border-surface-border pt-3">
              <p class="text-xs text-ink-tertiary mb-2">💡 Improve your score:</p>
              <ul class="space-y-1">
                <For each={q().improvements}>
                  {(tip: string) => (
